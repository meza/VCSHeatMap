...
public class ScoreCalculator {
	...
	public ScoreList getScoreFor(List<Map<String, String>> history) {
		...
		adjustScoreRegardingCollaborators(score, collaborators);
		...
		for (Map.Entry<String, Integer> scoreEntries : scoreTable.entrySet()) {
			if (!changeset.containsKey(scoreEntries.getKey())) {
				if (scoreEntries.getValue() <= 1) {
					...
					collabs.remove(scoreEntries.getKey());
				} else {
	...
	private void adjustScoreRegardingCollaborators(Map<String, Integer> scoreTable, Map<String, Set<String>> collabs) {
		for (Map.Entry<String, Integer> scoreEntries : scoreTable.entrySet()) {
